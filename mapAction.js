...
for (var i=0; i<data.length; i++) {
  locationContents += "<a id='marker" + data[i].id + "'></a><div class='location-summary' data-hub-id='" + data[i].id + "'>";
  locationContents += "<h3>" + data[i].name + "</h3>";
  locationContents += "<p>" + data[i].location + "</p>";
  locationContents += "<p><a href='"+data[i].website+"' target='blank'>" + data[i].website + "</a></p>";
  locationContents += data[i].phone ? "<p>" + data[i].phone + "</p>" : "";
	locationContents += data[i].email ? "<p><a href='mailto:"+data[i].email+"'>"  + data[i].email + "</a></p>" : "";
  locationContents += "</div>";
};
...
for (var i=0; i<data.length; i++) {
  var myCoordinates = data[i].coordinates.split(",")
  var lat = parseFloat(myCoordinates[0]);
  var long = parseFloat(myCoordinates[1]);
  var popupContent = "<p>"+ data[i].name +"</p>";
  markers[i] = L.marker([lat,long]).addTo(mymap).on('click', onMapClick).bindPopup(popupContent).openPopup();
  markers[i].myId = data[i].id;
}

function onMapClick(e) {
	$(".location-summary").removeClass('active')
	location.href = "#marker" + e.target.myId;
	$("#marker" + e.target.myId).next('.location-summary').addClass("active");
	printDetails(e.target.myId);
}

function getHubTarget(hubObjectId) {
	var hubTarget;
	data.forEach(function (hub) {
		if (hubObjectId == hub.id) {
			hubTarget = hub;
		}
	});
	return hubTarget;
...
function printDetails(hubObjectId) {
	var hubIndex;
	for (var i=0; i<data.length; i++) {
		if (data[i].id == hubObjectId) {
			hubIndex = i;
		}
	}
	var detailContents = "";
	detailContents += "<section class='hub' data-hub-id='" + data[hubIndex].id + "'>";
	detailContents += "<div class='info'>";
	detailContents += "<h3>" + data[hubIndex].name + "</h3>";
	detailContents += "<p>" + data[hubIndex].location + "</p>";
	detailContents += "<p><a href='"+data[hubIndex].website+"' target='blank'>" + data[hubIndex].website + "</a></p>";
	detailContents += data[hubIndex].phone ? "<p>" + data[hubIndex].phone + "</p>" : "";
	detailContents += data[hubIndex].email ? "<p><a href='mailto:"+data[hubIndex].email+"'>"  + data[hubIndex].email + "</a></p>" : "";
	detailContents += "</div>";
...
function flyAndPrintToHub(hubObjectId) {
  ...
  var myCoordinates = hubTarget.coordinates.split(",")
  var lat = parseFloat(myCoordinates[0]);
  var long = parseFloat(myCoordinates[1]);
  printDetails(hubObjectId);
  mymap.flyTo([lat,long], 13.5);
  for (var i=0; i<markers.length; i++) {
    if (markers[i].myId == hubTarget.id) {
